...
public class Stream_05_Test {
    ...
    private long iterateSum(long n) {
        return Stream.iterate(0, i -> i + 1).limit(n).collect(Collectors.summingLong(i -> i));
    }
    ...
    private long parallelIterateSum(long n) {
    	return Stream.iterate(0, i -> i + 1).parallel().limit(n).collect(Collectors.summingLong(i -> i));
    }
    ...
    public void monitor_imperativeSum_vs_iterateSum_vs_parallelIterateSum() {
        Logger.getAnonymousLogger().info("imperativeSum => " + monitor(n -> imperativeSum(n), 100000) + " ms");
        Logger.getAnonymousLogger().info("iterateSum => " + monitor(n -> iterateSum(n), 100000) + " ms");
        Logger.getAnonymousLogger().info("parallelIterateSum => " + monitor(n -> parallelIterateSum(n), 100000) + " ms");
    }
    ...
    public void monitor_imperativeSum_vs_iterateSum_vs_parallelIterateSum_vs_rangeSum_vs_rangeParallelSum() {
        ...
        Logger.getAnonymousLogger().info("iterateSum => " + monitor(n -> iterateSum(n), 100000) + " ms");
        Logger.getAnonymousLogger().info("parallelIterateSum => " + monitor(n -> parallelIterateSum(n), 100000) + " ms");
        Logger.getAnonymousLogger().info("rangeSum => " + monitor(n -> rangeSum(n), 100000) + " ms");
        Logger.getAnonymousLogger().info("rangeParallelSum => " + monitor(n -> rangeParallelSum(n), 100000) + " ms");
    }
